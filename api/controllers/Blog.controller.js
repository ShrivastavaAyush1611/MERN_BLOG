...
export const addBlog = async (req, res, next) => {
    try {
        ...
        if (!category) {
            return next(handleErrors(400, "Invalid category."));
        }
        let featuredImage = ''
        if (req.file) {
            // Upload an image
...
export const editBlog = async (req, res, next) => {
  try {
    ...
    const blog = await Blog.findById(blogid).populate('category', '_id name'); // Populate category ID and name
    if (!blog) {
...
export const getRelatedBlog = async (req, res, next) => {
    try {
        ...
        const relatedBlog = await Blog.find({ category: categoryId, slug: { $ne: blog } }).lean().exec() //This ensures that the slug field of the documents does not equal the value of the blog variable.
        res.status(200).json({
            relatedBlog
        })
    } catch (error) {
